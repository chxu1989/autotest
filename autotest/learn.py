# -*- coding:utf-8 -*-
import re
import time,re,pymysql,json,urllib,unittest
...
class ApiFlow(unittest.TestCase):
	...
	def test_readSQLcase(self):  # 读取数据库中相应的接口用例数据
		...
		coon = pymysql.connect( user='root', passwd='test@123', db='autotest', port=3306, host='127.0.0.1', charset='utf8' )
		cursor = coon.cursor()
	...
	def writeBug(bug_id, interface_name, request, response, res_check):
		...
		bugdetail = '[请求数据]<br />' + request.decode() + '<br/>' + '[预期结果]<br/>' \
				+ res_check.decode() + '<br/>' + '<br/>' + '[响应数据]<br />' + '<br/>' + response.decode()
		print( bugdetail )
		sql = "INSERT INTO `bug_bug` (`bugname`,`bugdetail`,`bugstatus`,`buglevel`, `bugcreater`,`bugassign`," \
		  " `created_time`, `Product_id`)VALUES( '%s', '%s', '1', '1', 'Sam', 'Sam', '%s','2');"%(bugname,pymysql.escape_string(bugdetail),now)
		coon =pymysql.connect( user='root', passwd='test@123', db='autotest', port=3306, host='127.0.0.1', charset='utf8')
		cursor = coon.cursor()
		cursor.execute( sql )
		coon.commit()
...
if __name__ == '__main__':
	...
	filename='E:\\Python37\Scripts\\autotest\\apitest\\templates\\apitest_report.html'
	fp=open(filename,'wb')
